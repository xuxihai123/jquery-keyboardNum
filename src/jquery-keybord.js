(function (window, $) {
  ...
  var keybordEnv = {
    ...
    setPubKeybord: function () {
      keybordEnv.$keybord = $(
        '<div class="keyboard-wrap" style="display: none">' +
          '<div class="mask"></div>' + //jquery Object
          '<div class="keyboard-container" >' +
          '   <div class="keyboard-val"></div>' +
    ...
    setPubCursor: function () {
      keybordEnv.$cursor = $(
        '<div class="keyboard-cursor" style="display: none"></div>'
      );
    ...
    bindPbEvent: function () {
      ...
      $keybord.delegate(".keyboard-arrow", "click", function (event) {
        //关闭键盘
      ...
      $keybord.delegate(
        ...
        function (event) {
          ...
          keybordEnv.$instance &&
            $(keybordEnv.$instance.$input).trigger(
              "keypress.keyboard",
              keyValue
            );
        }
  ...
  KeybordInput.prototype.$getCursorPos = function () {
    //获得input光标位置
  ...
  KeybordInput.prototype.$validate = function (inputValue) {
    var options = this.$options,
      result = true;
    if (typeof options.validateHook == "function") {
      //使用自定义的校验规则
    ...
    if (NUMBER_REGEXP.test(inputValue)) {
      ...
      if (index > 0) {
        ...
        if (dotString.length > options.dotLength) {
          // Render the last valid input in the field
          result = false;
        } else if (intString.length > options.intLength) {
          result = false;
        }
      } else if (inputValue.length > options.intLength) {
        // Render the last valid input in the field
        result = false;
      }
    } else {
  ...
  KeybordInput.prototype.$handleKeyValue = function (keyValue) {
    ...
    if (keyValue === "delete" && this.$cursorPosition > 0) {
      if (inputValue && inputValue.length > 0) {
        newValue =
          inputValue.substr(0, this.$cursorPosition - 1) +
          inputValue.substr(this.$cursorPosition, inputValue.length);
      ...
      if (inputValue && inputValue.length > 0) {
        newValue =
          inputValue.substr(0, this.$cursorPosition) +
          keyValue +
          inputValue.substr(this.$cursorPosition, inputValue.length);
      } else {
        newValue = keyValue;
